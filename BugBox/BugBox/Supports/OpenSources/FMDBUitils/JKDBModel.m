...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
         ...
         */
        DLog(@"propertyType:%@",propertyType);
        if ([propertyType hasPrefix:@"T@"]) {
            [proTypes addObject:SQLTEXT];
        } else if ([propertyType hasPrefix:@"Ti"]||[propertyType hasPrefix:@"TI"]||[propertyType hasPrefix:@"Ts"]||[propertyType hasPrefix:@"TS"]||[propertyType hasPrefix:@"TB"]||[propertyType hasPrefix:@"Tq"]) {
...
//add by maxmoo
+ (NSString *)findFirstBySQLString:(NSString *)SQLString andColumeName:(NSString *)columeNameString{
    NSArray *results = [self.class findBySQLString:SQLString andColumeName:columeNameString];
    if (results.count < 1) {
        return nil;
    }
    return [results firstObject];
...
//add by maxmoo
+ (NSArray *)findBySQLString:(NSString *)SQLString andColumeName:(NSString *)columeNameString{
    JKDBHelper *jkDB = [JKDBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ...
            string = [resultSet stringForColumn:columeNameString];
            [users addObject:string];
            FMDBRelease(model);
        }
    }];
    return users;
